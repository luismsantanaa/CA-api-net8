...
namespace AppApi.Configuration
{
    ...
    public static class ConfigurationValidator
    {
        ...
        public static void ValidateConfiguration(IConfiguration configuration, ILogger? logger = null)
        {
            ...
            // Validate Connection Strings (Critical if not using InMemory)
            ...
            {
                logger?.LogInformation("Skipping connection string validation - Using InMemory database");
            }
        ...
        private static void ValidateConnectionStrings(IConfiguration configuration, List<string> errors)
        {
            ...
            {
                errors.Add("ConnectionStrings:ApplicationConnection is required when not using InMemory database");
            }

            var identityConnection = connectionStrings["IdentityConnection"];
            if (string.IsNullOrWhiteSpace(identityConnection))
            {
                errors.Add("ConnectionStrings:IdentityConnection is required when not using InMemory database");
            }
        }
